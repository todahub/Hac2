...
* **ID $N$**: **64回** 送信し続けた後、停止ボタンが押されるまでは ID $N$ を維持します。
* **停止ボタン押下後**: ホストは ID 1 を送出し続け、クライアント側は ID 4 の後に受信した ID 1 を停止要求として扱います。

> [!NOTE]
> テスト版スケッチでは、クライアント台数定数は `NUM_CLIENTS = 2` に設定されています。
> そのため、通常時は **「ID 1 を8回送信 ➔ ID 2 を64回送信 ➔ ID 2 を維持」** という動作になります。停止ボタンを押すとホストは ID 1 を固定送出し、クライアントはその ID 1 を停止要求として解釈します。本番環境（4台）では定数を `4` に変更してください。
...
3. 可変抵抗を回して、BPM が変化すること、および発信されるIDが「1（8回）➔ 2（64回）➔ 1...」と循環することを確認します。
